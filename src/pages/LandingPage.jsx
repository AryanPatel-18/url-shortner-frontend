function LandingPage({ onLogin, onRegister }) {
  return (
    <main className="relative overflow-hidden">
      <div className="pointer-events-none absolute left-1/2 top-0 h-96 w-96 -translate-x-1/2 rounded-full bg-blue-100/60 blur-3xl" aria-hidden="true" />
      <section className="relative mx-auto grid max-w-6xl items-center gap-14 px-4 pb-20 pt-16 sm:px-6 sm:pt-24 lg:grid-cols-[1.05fr_0.95fr] lg:px-8 lg:pb-28">
        <div>
          <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.16em] text-brand">
            <span className="h-1.5 w-1.5 rounded-full bg-brand" aria-hidden="true" />
            Simple links, ready to share
          </div>
          <h1 className="max-w-3xl text-5xl font-black leading-[1.02] tracking-[-0.055em] text-ink sm:text-6xl lg:text-7xl">
          ...
          <div className="mt-9 flex flex-col gap-3 sm:flex-row">
            ...
            <button type="button" onClick={onLogin} className="rounded-2xl border border-slate-200 bg-white px-6 py-3.5 text-sm font-bold text-ink transition hover:border-slate-300 hover:bg-slate-50 focus:outline-none focus:ring-4 focus:ring-brand/10">Sign in</button>
          </div>
        </div>

        <div className="relative">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-[0_24px_80px_-32px_rgba(23,32,51,0.35)] sm:p-7">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Your workspace</p>
                <p className="mt-1 text-lg font-bold text-ink">Link overview</p>
              </div>
              <span className="grid h-10 w-10 place-items-center rounded-2xl bg-blue-50 text-brand" aria-hidden="true">↗</span>
            </div>
            <div className="space-y-3">
              {[
                ...
                <div key={short} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <p className="truncate text-xs text-slate-500">{original}</p>
                  <div className="mt-2 flex items-center justify-between gap-3">
                    <p className="truncate text-sm font-bold text-brand">{short}</p>
                    <p className="shrink-0 text-xs font-semibold text-slate-400">{clicks}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="absolute -bottom-5 -left-4 hidden rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-lg sm:block">
            ...
            <p className="mt-1 text-sm font-bold text-ink">Fast. Focused. Yours.</p>
          </div>
        </div>
      </section>
