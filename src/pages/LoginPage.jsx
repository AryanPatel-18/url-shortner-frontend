    ...
    <main className="mx-auto flex min-h-[calc(100svh-73px)] w-full max-w-6xl items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
      <div className="grid w-full max-w-4xl overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-[0_24px_80px_-32px_rgba(23,32,51,0.28)] lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <div className="p-6 sm:p-10">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">Use your account to access your URL library.</p>

          <div className="mt-6"><ErrorBanner message={error} onDismiss={() => setError('')} /></div>

          <form onSubmit={handleSubmit} className="mt-6 space-y-5">
            <div>
              <label htmlFor="login-email" className="mb-2 block text-sm font-semibold text-ink">Email</label>
            ...
            <div>
              <label htmlFor="login-password" className="mb-2 block text-sm font-semibold text-ink">Password</label>
              <input id="login-password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-sm text-ink outline-none transition placeholder:text-slate-400 focus:border-brand focus:bg-white focus:ring-4 focus:ring-brand/10" placeholder="Your password" disabled={submitting} />
            </div>
          ...
          <p className="mt-7 text-center text-sm text-slate-500">Need an account? <button type="button" onClick={onRegister} className="font-bold text-brand hover:underline">Create one</button></p>
        </div>
      </div>
    </main>
  )
